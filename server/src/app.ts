...
// SECURITY: Global hook to add standard security headers
// Mitigates missing security headers finding
app.addHook('onRequest', (request, reply, done) => {
  reply.header('X-Content-Type-Options', 'nosniff');
  reply.header('X-Frame-Options', 'DENY');
  reply.header('X-XSS-Protection', '1; mode=block');
  reply.header('Strict-Transport-Security', 'max-age=31536000; includeSubDomains');
  // Optional CSP, kept simple to avoid breaking SPA inline scripts/styles
  reply.header('Content-Security-Policy', "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: https:");
  done();
});
