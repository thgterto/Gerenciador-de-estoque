...
// Polaris-like Grid Template (Must match InventoryTable header)
const GRID_TEMPLATE = "40px minmax(240px, 3fr) 120px minmax(180px, 1.5fr) 100px 100px 130px 110px"; // Ajustado largura ações
    ...
    return (
        <div style={style} className="w-full">
            <div className={`
                ...
                ${isLast ? 'border-b-0' : ''}
            `}>
                {/* Visual Connection Line for Tree Structure */}
                <div className="absolute left-[28px] top-0 bottom-1/2 w-px bg-border-light dark:border-border-dark"></div>
                <div className="absolute left-[28px] top-1/2 w-3 h-px bg-border-light dark:border-border-dark"></div>

                <div className="grid h-full items-center text-sm py-0 pl-12 pr-4" style={{ gridTemplateColumns: GRID_TEMPLATE }}>
                    
                    {/* Lote / ID */}
                    <div className="px-2 flex items-center gap-3 overflow-hidden">
                         <div className="flex flex-col gap-0.5 min-w-0">
                             <div className="flex items-center gap-2">
                                <span className="material-symbols-outlined text-[16px] text-text-light">qr_code_2</span>
                                <Tooltip content={`Lote: ${item.lotNumber}`} position="top">
                                    <span 
                                        className="font-mono text-xs font-medium text-text-main dark:text-slate-300 cursor-copy hover:text-primary transition-colors"
                    ...
                    {/* Category Placeholder (Empty for child) */}
                    ...
                    <div className="px-2 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                             <Tooltip content="Novo Lote (Clonar)">
                                <button onClick={()=>onActions.clone(item)} className="p-1.5 hover:bg-background-light dark:hover:bg-slate-700 rounded text-text-secondary hover:text-primary transition-colors">
                                    <span className="material-symbols-outlined text-[16px]">content_copy</span>
                                </button>
                             </Tooltip>
                             <Tooltip content="Editar">
                                <button onClick={()=>onActions.edit(item)} className="p-1.5 hover:bg-background-light dark:hover:bg-slate-700 rounded text-text-secondary hover:text-primary transition-colors">
                                    <span className="material-symbols-outlined text-[16px]">edit</span>
                                </button>
                             </Tooltip>
                             <Tooltip content="Movimentar">
                                <button onClick={()=>onActions.move(item)} className="p-1.5 hover:bg-background-light dark:hover:bg-slate-700 rounded text-text-secondary hover:text-primary transition-colors">
...
export const InventoryGroupRow = React.memo(({ 
    ...
    return (
        <div style={style} className="w-full">
            <div className={`
                h-full border-b border-border-light dark:border-border-dark transition-colors cursor-pointer
                ${isExpanded ? 'bg-background-light dark:bg-slate-800/30' : 'bg-surface-light dark:bg-surface-dark hover:bg-background-light dark:hover:bg-slate-800/20'}
                ...
                <div className="grid h-full items-center px-4 text-sm py-0" style={{ gridTemplateColumns: GRID_TEMPLATE }}>
                    ...
                    <div className="px-2 overflow-hidden">
                        <div className="flex items-center gap-3">
                             <div className={`p-1.5 rounded-lg border border-border-light dark:border-border-dark ${isExpanded ? 'bg-surface-light dark:bg-slate-700 text-primary' : 'bg-background-light dark:bg-slate-800 text-text-secondary'} transition-colors`}>
                                <span className="material-symbols-outlined text-[20px]">{getCategoryIcon(primaryItem.category)}</span>
                             </div>
                             <div className="flex flex-col min-w-0">
                                 <div 
                                    className="font-semibold text-text-main dark:text-white truncate text-sm"
                                 ...
                                 <div className="flex items-center gap-2 mt-0.5">
                                    <span 
                                        className="text-[11px] font-mono text-text-secondary dark:text-slate-400 cursor-copy hover:text-primary transition-colors bg-background-light dark:bg-slate-700/50 px-1.5 rounded"
                                        onClick={(e) => { e.stopPropagation(); copyToClipboard(primaryItem.sapCode, 'SKU'); }}
                                    >
                                        {primaryItem.sapCode || 'N/A'}
                                    </span>
                                    {items.length > 1 && (
                                        <span className="text-[10px] px-1.5 py-px rounded-full bg-border-light dark:bg-slate-700 text-text-secondary dark:text-slate-300 font-bold">
                                            {items.length}
                                        </span>
                                    )}
                                 </div>
                    ...
                    {/* Expand Icon */}
                    <div className="px-2 text-right">
                         <span className={`material-symbols-outlined text-text-light transition-transform duration-200 ${isExpanded ? 'rotate-90 text-primary' : ''}`}>
                             chevron_right
                         </span>
                    </div>
                </div>
        ...
        <div style={style} className="w-full px-4 pt-4 pb-1">
            ...
            >
                <div className="p-3 flex items-start gap-3">
                    {/* Icon Box */}
                    <div className={`
                        shrink-0 size-10 rounded-lg flex items-center justify-center border
                        ${isExpanded 
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                             <h4 className="font-bold text-text-main dark:text-white truncate text-sm pr-2">
                                 {primaryItem.name}
                             </h4>
                             {items.length > 1 && (
                                 <span className="text-[10px] font-bold bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-text-secondary">
                                     {items.length}
                                 </span>
                             )}
                        </div>
                ...
                `}>
                    <div className="flex items-center gap-1 text-text-secondary dark:text-slate-400">
                        ...
                        <span className="max-w-[120px] truncate">{group.locations.join(', ')}</span>
                    </div>
                    <div className="font-bold text-text-main dark:text-white">
                        {totalQuantity} <span className="font-normal text-text-light">{primaryItem.baseUnit}</span>
                    </div>
                </div>
            </div>
        </div>
...
export const InventoryMobileChildRow = React.memo(({ 
    ...
    return (
        <div style={style} className="w-full px-4 pl-8 pb-2 relative">
             <div className="absolute left-[24px] top-0 bottom-0 w-px bg-border-light dark:border-slate-700"></div>
             <div className="absolute left-[24px] top-[24px] w-3 h-px bg-border-light dark:border-slate-700"></div>

             <div className="bg-white dark:bg-slate-800 rounded-lg border border-border-light dark:border-slate-700 p-3 shadow-sm relative">
                {/* Lote Header */}
                <div className="flex justify-between items-start mb-2">
                    <div>
                        <div className="flex items-center gap-1.5">
                            <span className="material-symbols-outlined text-[16px] text-primary">qr_code_2</span>
                            <span className="font-mono text-sm font-bold text-text-main dark:text-white">{item.lotNumber}</span>
                        </div>
                        <span className="text-[10px] text-text-secondary dark:text-gray-400 block mt-0.5 ml-0.5">
                    ...
                    <div className="text-right">
                         ...
                         <div className={`text-[10px] font-medium mt-0.5 ${status.isExpired ? 'text-red-500' : 'text-text-light'}`}>
                             {validityInfo}
                         </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 mt-3 pt-2 border-t border-dashed border-slate-100 dark:border-slate-700">
                     <button onClick={()=>onActions.move(item)} className="flex flex-col items-center justify-center text-text-secondary hover:text-primary py-1 active:bg-slate-50 dark:active:bg-slate-700 rounded">
                         <span className="material-symbols-outlined text-[18px]">swap_horiz</span>
                         <span className="text-[9px] mt-0.5 font-medium">Mover</span>
                     </button>
                     <button onClick={()=>onActions.clone(item)} className="flex flex-col items-center justify-center text-text-secondary hover:text-primary py-1 active:bg-slate-50 dark:active:bg-slate-700 rounded">
                         <span className="material-symbols-outlined text-[18px]">content_copy</span>
                         <span className="text-[9px] mt-0.5 font-medium">Clonar</span>
                     </button>
                     <button onClick={()=>onActions.edit(item)} className="flex flex-col items-center justify-center text-text-secondary hover:text-primary py-1 active:bg-slate-50 dark:active:bg-slate-700 rounded">
                         <span className="material-symbols-outlined text-[18px]">edit</span>
                         <span className="text-[9px] mt-0.5 font-medium">Editar</span>
                     </button>
                     <button onClick={()=>onActions.qr(item)} className="flex flex-col items-center justify-center text-text-secondary hover:text-primary py-1 active:bg-slate-50 dark:active:bg-slate-700 rounded">
