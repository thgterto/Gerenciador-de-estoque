...
function upsertData(sheetName, headers, items, idField = 'id') {
  ...
  items.forEach(item => {
    ...
    const row = headers.map(h => {
        ...
        if (h === 'Category') return sanitizeForSheets(item.categoryId);
        ...
        if (h === 'LocationID') return sanitizeForSheets(item.locationId);
        if (h === 'Quantity') return item.quantity;
        
        // History specific
        if (h === 'Date') return item.date;
        if (h === 'Type') return item.type;
        if (h === 'User') return sanitizeForSheets(item.userId || 'Sistema');
...
function deleteItem(itemId) {
  // Security Guard: Prevent deleting all items with empty ID
  if (itemId === undefined || itemId === null || String(itemId).trim() === '') {
      console.error("Security Block: Attempted to delete with empty ID");
      return;
  }
