...
export const useInventoryFilters = (items: InventoryItem[]) => {
    ...
    const baseFilteredItems = useMemo(() => {
        ...
        return items.filter(i => {
            ...
            // Filtro de Status
            if (statusFilter !== 'ALL') {
                const status = getItemStatus(i); // Bolt: Omitting now leverages fast string comparison
    ...
    const groupedInventory = useMemo(() => {
        ...
        const result = Object.values(groups).map(grp => {
            ...
            for (const i of grp.items) {
                const s = getItemStatus(i); // Bolt: Omitting now leverages fast string comparison
