    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            printWindow.document.write(`
                <html>
                    <head>
                        <title>${sanitizeHTML(item.name)}</title>
                        <style>
                    ...
                    <body>
                        <div class="label">
                            <div class="title">${sanitizeHTML(item.name)}</div>
                            <div class="meta">Lote: ${sanitizeHTML(item.lotNumber)} | Val: ${sanitizeHTML(item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A')}</div>
                            ${svgHtml}
                            <div class="meta" style="margin-top: 5px;">${sanitizeHTML(item.id)}</div>
                        </div>
