...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            title.textContent = item.name || 'Print';
            ...
            const exp = item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A';
            meta1.textContent = `Lote: ${item.lotNumber || ''} | Val: ${exp}`;
            label.appendChild(meta1);

            const svgNode = document.getElementById('qr-code-svg');
            if (svgNode) {
                label.appendChild(doc.importNode(svgNode, true));
            }

            const meta2 = doc.createElement('div');
            meta2.className = 'meta';
            meta2.style.marginTop = '5px';
            meta2.textContent = item.id || '';
