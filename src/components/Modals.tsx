...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        const safeExpiryDate = item.expiryDate ? escapeHtml(new Date(item.expiryDate).toLocaleDateString()) : 'N/A';
        ...
        const printWindow = window.open(url, '', 'width=600,height=400');
        if (printWindow) {
            // Give browser time to load blob and execute print then revoke object URL
