...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            // Generate basic HTML structure with safe static content
            ...
            // Safely inject dynamic content using DOM methods to prevent XSS
            printWindow.document.title = item.name || 'QR Code';

            const labelContainer = printWindow.document.getElementById('label-container');
            if (labelContainer) {
                ...
                metaDiv1.textContent = `Lote: ${item.lotNumber} | Val: ${item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A'}`;
                labelContainer.appendChild(metaDiv1);

                const svgWrapper = printWindow.document.createElement('div');
                svgWrapper.innerHTML = svgHtml; // Safe because we control the SVG content generated by QRCode
                labelContainer.appendChild(svgWrapper);
                ...
                labelContainer.appendChild(metaDiv2);
            }
        }
    };
