...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            // XSS Prevention: Sanitize user inputs before injecting into HTML
            ...
            const safeName = item.name ? escapeHtml(item.name) : '';
            const safeLot = item.lotNumber ? escapeHtml(item.lotNumber) : '';
            const safeId = item.id ? escapeHtml(item.id) : '';
                ...
                <html>
                    ...
                    <body>
                        <div class="label">
                            ...
                            <div class="meta">Lote: ${safeLot} | Val: ${item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A'}</div>
                            ${svgHtml}
