        ...
        <div className="pb-24">
            {visibleItems.map((rowItem: any, index: number) => {
                ...
                if (rowItem.type === 'GROUP') {
                    const items = rowItem.data.items;
                    const allSelected = items.length > 0 && items.every((i: any) => selectedIds.has(i.id));
                    const someSelected = items.some((i: any) => selectedIds.has(i.id));
