...
export const OrbitalInput = forwardRef<HTMLInputElement, OrbitalInputProps>(({
    ...
    return (
        <div className={`flex flex-col gap-1.5 ${fullWidth ? 'w-full' : ''}`}>
            {label && (
                <label htmlFor={inputId} className="text-xs font-display font-bold uppercase tracking-wider text-orbital-subtext pl-1 cursor-pointer hover:text-orbital-text transition-colors">
                    {label}
                </label>
            )}
