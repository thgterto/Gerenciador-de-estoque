...
export const Purchases: React.FC<Props> = ({
  ...
  const recommendations = useMemo(() => {
      if (!items) return [];
      // ⚡ Bolt: Cache Date.now() and avoid repeating Date instantiation in loop
      const now = Date.now();
      return items.filter(i => {
          const isLow = i.quantity <= i.minStockLevel;
          const daysToExpiry = i.expiryDate ? Math.ceil((new Date(i.expiryDate).getTime() - now) / 86400000) : 999;
