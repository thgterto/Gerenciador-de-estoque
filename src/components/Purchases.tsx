...
export const Purchases: React.FC<Props> = ({
  ...
  const recommendations = useMemo(() => {
      ...
      // Optimization: Cache current time and convert purchaseList to Set for O(1) lookup
      const nowTime = Date.now();
      const purchaseListIds = new Set(purchaseList.map(p => p.id));
      const msPerDay = 1000 * 60 * 60 * 24;

      return items.filter(i => {
          const isLow = i.quantity <= i.minStockLevel;
          const daysToExpiry = i.expiryDate ? Math.ceil((new Date(i.expiryDate).getTime() - nowTime) / msPerDay) : 999;
          const isExpiring = daysToExpiry < 30;
          const alreadyInList = purchaseListIds.has(i.id);
            ...
            <div className="mb-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 overflow-x-auto pb-2">
                    {recommendations.map(item => {
                        const daysToExpiry = item.expiryDate ? Math.ceil((new Date(item.expiryDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24)) : 999;
