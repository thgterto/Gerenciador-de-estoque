...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            // 🛡️ Sentinel: Sanitize user input before injecting into document.write to prevent XSS
            const safeName = escapeHtml(item.name || '');
            const safeLot = escapeHtml(item.lotNumber || '');
            const safeId = escapeHtml(item.id || '');
            const safeExpiry = item.expiryDate ? escapeHtml(new Date(item.expiryDate).toLocaleDateString()) : 'N/A';
