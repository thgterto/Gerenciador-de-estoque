...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            // Avoid document.write to prevent XSS
            ...
            const titleEl = printWindow.document.createElement('div');
            titleEl.className = 'title';
            titleEl.textContent = item.name;
            container.appendChild(titleEl);

            const meta1El = printWindow.document.createElement('div');
            meta1El.className = 'meta';
            const expDateStr = item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A';
            meta1El.textContent = `Lote: ${item.lotNumber} | Val: ${expDateStr}`;
            container.appendChild(meta1El);

            const svgWrapper = printWindow.document.createElement('div');
            svgWrapper.innerHTML = svgHtml;
            container.appendChild(svgWrapper);

            const meta2El = printWindow.document.createElement('div');
            meta2El.className = 'meta';
            meta2El.style.marginTop = '5px';
            meta2El.textContent = item.id;
            container.appendChild(meta2El);
