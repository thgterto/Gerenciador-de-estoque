...
export const QRGeneratorModal: React.FC<QRGeneratorModalProps> = ({ isOpen, onClose, item }) => {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            const dateStr = item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A';
            metaDiv1.textContent = `Lote: ${item.lotNumber || ''} | Val: ${dateStr}`;
            labelDiv.appendChild(metaDiv1);

            if (svgNode) {
                // clone the SVG node to append it safely
                labelDiv.appendChild(svgNode.cloneNode(true));
            }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <OrbitalSelect
                    ...
                    options={filteredItems.map(item => ({
                        value: item.id,
                        label: `${escapeHtml(item.name)} (${item.quantity} ${item.baseUnit})`
                    }))}
                    fullWidth
                    required
                />
