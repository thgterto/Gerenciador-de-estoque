...
export const ImportService = {
  ...
  async importBulk(
    ...
    overwriteQuantities: boolean = false,
    ...
    try {
      ...
      if (replaceMode) {
        ...
        await db.transaction(
          ...
          async () => {
            ...
            if (v2Data.catalog.length > 0)
              await db.rawDb.catalog.bulkAdd(v2Data.catalog);
            if (v2Data.batches.length > 0)
              await db.rawDb.batches.bulkAdd(v2Data.batches);
            if (v2Data.partners.length > 0)
              await db.rawDb.partners.bulkAdd(v2Data.partners);
            if (v2Data.locations.length > 0)
              await db.rawDb.storage_locations.bulkAdd(v2Data.locations);
            if (v2Data.balances.length > 0)
              await db.rawDb.balances.bulkAdd(v2Data.balances);
          },
        );
      ...
      processedItems.forEach((newItem) => {
        ...
        if (existing) {
          ...
          const merged: InventoryItem = {
            ...
            molecularFormula:
              newItem.molecularFormula || existing.molecularFormula,
            risks: hasActiveRisks(newItem.risks)
              ? newItem.risks
              : existing.risks,
            // Preserva saldo existente a menos que seja um reset explícito
            quantity:
              overwriteQuantities && newItem.quantity !== undefined
                ? newItem.quantity
                : existing.quantity,
          };
      ...
      await db.transaction(
        ...
        async () => {
          ...
          if (v2Data.catalog.length > 0)
            await db.rawDb.catalog.bulkPut(v2Data.catalog);
          if (v2Data.batches.length > 0)
            await db.rawDb.batches.bulkPut(v2Data.batches);
          if (v2Data.partners.length > 0)
            await db.rawDb.partners.bulkPut(v2Data.partners);
          if (v2Data.locations.length > 0)
            await db.rawDb.storage_locations.bulkPut(v2Data.locations);

          if (v2Data.balances.length > 0)
            await db.rawDb.balances.bulkPut(v2Data.balances);
        },
      );
  ...
  async importHistoryBulk(
    history: MovementRecord[],
    updateStockBalance: boolean = false,
  ): Promise<ImportResult> {
    const stats = {
    ...
    try {
      await db.transaction(
        "rw",
        [db.rawDb.history, db.rawDb.items, db.rawDb.balances],
        async () => {
          ...
          if (updateStockBalance) {
            ...
            history.forEach((h) => {
              ...
              if (h.type === "ENTRADA")
                qtyMap.set(h.itemId, current + h.quantity);
            ...
            items.forEach((item, idx) => {
              ...
              if (!locationId) {
                const locStr =
                  `${item.location.warehouse || "Geral"} ${item.location.cabinet || ""} ${item.location.shelf || ""}`.trim();
            ...
            if (updatedBalances.length > 0)
              await db.rawDb.balances.bulkPut(updatedBalances);
          }
        },
      );
  ...
  async enrichInventory(
    ...
    const candidates = await db.rawDb.items
      .filter(
        (i) =>
          !!(i.casNumber && i.casNumber.length > 4) &&
          (!i.molecularFormula || !hasActiveRisks(i.risks)),
      )
      .toArray();
    ...
    const results = await CasApiService.fetchBatchChemicalData(
      casList,
      (curr, tot) => {
        onProgress(curr, tot);
      },
    ...
    candidates.forEach((item) => {
      ...
      if (data) {
        ...
        updates.push({
          ...
          risks: hasActiveRisks(item.risks)
            ? item.risks
            : ({ ...item.risks, ...suggestedRisks } as RiskFlags),
        });
        updatedCount++;
      }
    });
    ...
    if (updates.length > 0) {
      try {
        ...
        await db.transaction(
          "rw",
          [db.rawDb.items, db.rawDb.catalog],
          async () => {
