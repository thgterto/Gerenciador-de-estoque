...
## 2026-07-09 - Missing Skip to Main Content Link
**Learning:** React single-page applications often lack a "skip to main content" link, forcing keyboard users to navigate through all header links on every view. This app was missing one in the main Layout component.
**Action:** Always add a visually hidden (but focusable) "Skip to main content" link at the very top of the DOM structure that targets the main `<main>` container ID.
