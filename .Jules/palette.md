...
## 2025-05-24 - Accessibility of SVG Icons in Buttons
**Learning:** Even when using SVG icons (like Lucide React components) inside buttons that already have an `aria-label` or visible text, screen readers might still announce the SVG's presence or title if not explicitly hidden. This leads to redundant or confusing announcements.
**Action:** Always add `aria-hidden="true"` to the SVG icon element itself when an accessible label is provided on the parent button.
