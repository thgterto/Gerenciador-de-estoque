...
## 2025-01-09 - Focus Visible on Custom Buttons
**Learning:** Custom button components often lose their native browser focus outlines due to utility classes or resets. If these outlines are not explicitly redefined using `focus-visible`, keyboard users lose their place on the page, significantly hurting accessibility.
**Action:** Always ensure that core interactive components (like `OrbitalButton`) explicitly define `focus-visible` styles (e.g., `focus-visible:ring-2`) to guarantee keyboard navigation is robust out of the box.
