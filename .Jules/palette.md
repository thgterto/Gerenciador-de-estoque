...
## 2025-05-25 - Dynamic Aria Labels
**Learning:** For buttons that convey state (like theme toggle or notifications), a static `aria-label` is insufficient. The label must reflect the current state (e.g., "Modo Escuro" vs "Modo Claro", "5 notificações não lidas").
**Action:** Use conditional logic in `aria-label` to provide accurate context to screen reader users, matching the visual feedback.
