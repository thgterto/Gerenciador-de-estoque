...
## 2025-02-14 - Item Status Optimization
**Learning:** Calling `getItemStatus(item, new Date())` inside loops or `useMemo` creates a new Date object for every item, and within `getItemStatus`, another `new Date(item.expiryDate)` is created if the second argument is passed. This is extremely slow for large lists. Removing the second argument allows `getItemStatus` to use its internal optimized string comparison (`item.expiryDate < getTodayISO()`), which is ~30x faster.
**Action:** Never pass `new Date()` to `getItemStatus` in a loop. Let the function use its internal optimized cache.
