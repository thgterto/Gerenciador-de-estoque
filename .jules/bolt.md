...
## 2024-05-22 - React.memo Limitations
**Learning:** `React.memo` will not work effectively if all the props passed down to it are not primitive or memoized (via `useCallback` or `useMemo`).
**Action:** When applying `React.memo`, ensure you follow up and confirm that any complex prop object or callback passed from the parent component is stable across re-renders.
