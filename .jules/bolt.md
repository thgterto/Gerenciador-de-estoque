...
## 2025-02-14 - NativeList Component Function Re-creation
**Learning:** In `src/components/inventory/InventoryList.tsx`, the `NativeList` component is rendering a long list of items using `Array.prototype.map`. However, inside the `map` function, an inline arrow function is being created on every render for `toggleExpand` prop in `InventoryMobileGroupRow` and `InventoryGroupRow`: `toggleExpand={() => toggleGroupExpand(rowItem.data.groupKey)}`. This breaks the `React.memo` on the row components because the `toggleExpand` function reference changes on every render of `NativeList`, causing all visible rows to re-render unnecessarily when `visibleCount` or `selectedIds` changes.
**Action:** Always avoid creating inline functions inside `map` for props passed to `React.memo` components. Create a wrapper component or use a stabilized callback that passes the `id` as an argument.
## 2025-02-14 - NativeList Component Function Re-creation
**Learning:** In `src/components/inventory/InventoryList.tsx`, the `NativeList` component is rendering a long list of items using `Array.prototype.map`. However, inside the `map` function, an inline arrow function is being created on every render for `toggleExpand` prop in `InventoryMobileGroupRow` and `InventoryGroupRow`: `toggleExpand={() => toggleGroupExpand(rowItem.data.groupKey)}`. This breaks the `React.memo` on the row components because the `toggleExpand` function reference changes on every render of `NativeList`, causing all visible rows to re-render unnecessarily when `visibleCount` or `selectedIds` changes.
**Action:** Always avoid creating inline functions inside `map` for props passed to `React.memo` components. Create a wrapper component or use a stabilized callback that passes the `id` as an argument.
