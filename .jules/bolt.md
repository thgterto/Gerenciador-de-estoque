...
## 2025-03-09 - Optimized ISO String Cache Bypassed by Explicit Date Passing
**Learning:** The `getItemStatus` function was heavily optimized to use string comparison with a cached "today" ISO string. However, passing an explicit `now` parameter (e.g. `const now = new Date(); getItemStatus(item, now)`) forces the function back onto the slow `new Date(item.expiryDate) < now` path. In filtering operations over large datasets, this effectively nullified the performance optimization, causing massive main-thread blocking by repeatedly parsing thousands of Date objects per render.
**Action:** Remove explicit `now` Date objects when calling utility functions like `getItemStatus` if they have internal caching mechanisms optimized for string comparisons, especially inside `filter` or `map` loops. Always prefer lexicographical string comparisons for ISO dates.
