...
## 2025-02-14 - Expensive Date Parsing inside render/loops
**Learning:** Initializing `new Date(item.expiryDate)` inside loops for comparison with other dates creates thousands of allocations per render in analytics and filters logic, heavily degrading performance.
**Action:** Replace `new Date(item.expiryDate) < new Date(now)` with fast ISO 8601 string comparisons (e.g. `item.expiryDate < getTodayISO()`). For ranges, use explicit string inequalities. Wait to extract dates into `Date` objects only when it is needed for rendering localized format strings.
