...
## 2025-03-05 - Added React.memo for History List components
**Learning:** React list child components (`HistoryMobileRow`, `HistoryRow`, `NativeHistoryList`) within `HistoryTable.tsx` were re-rendering unnecessarily. Using `React.memo` for list row components in React applications optimizes performance by preventing full re-renders of the list when parent state changes.
**Action:** Always check list row components in React applications and wrap them in `React.memo` if they do not depend on external state/context that isn't passed via props.
