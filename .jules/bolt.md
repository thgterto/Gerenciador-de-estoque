...
## 2025-02-14 - Date Sorting Optimization (Re-confirmed)\n**Learning:** Re-confirmed that replacing `new Date(str).getTime()` with lexicographical string comparisons for ISO 8601 strings in `.sort()` loops leads to significant performance boosts.\n**Action:** Applied this optimization in `InventoryService.ts` for batch sorting, achieving ~38x speedup in tests.
