...
## 2024-05-18 - Virtualized List Optimization with Dynamic Measurements\n**Learning:** When virtualizing lists with dynamic or varying row heights using `react-window` (`VariableSizeList`), caching the actual rendered height is essential. However, calling `resetAfterIndex` carelessly on every render can cause infinite loops or severe performance degradation.\n**Action:** Always wrap the measurement update inside a `useEffect` bounded to specific `rowItem` data changes, and ideally, check if the measured height differs from the cached height (`if (sizeMap.current[index] !== newSize)`) before triggering `resetAfterIndex`.
