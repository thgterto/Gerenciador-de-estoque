...
## 2025-02-14 - Render Loop Object Allocation
**Learning:** Calling `new Date().getTime()` repeatedly inside tight render loops (like `map` or `filter` on arrays) causes significant unnecessary object allocations and GC pauses, blocking the main thread.
**Action:** Always hoist `new Date().getTime()` or other expensive, deterministic initializations to a single shared variable outside the loop.
