...
## 2025-02-14 - Redundant Date instantiation in loops
**Learning:** `new Date().getTime()` was being called repeatedly inside React render loops (`.map()`), which created throwaway `Date` objects and caused unnecessary garbage collection pressure, as well as degraded render performance.
**Action:** When computing current timestamp inside loops, replace `new Date().getTime()` with `Date.now()`, or hoist it outside the loop to be evaluated only once.
