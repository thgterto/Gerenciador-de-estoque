...
## 2025-03-05 - Date Parsing and Evaluation in Loops
**Learning:** Constructing `Date` objects repeatedly via `new Date(dateString).getTime()` inside loops (`.map`, `.filter`, `.sort`) allocates memory rapidly and causes GC pressure, especially when calculating differences with `new Date().getTime()`. In tests, it causes noticeable slowdown for large arrays.
**Action:** Always use `Date.parse(dateString)` to calculate timestamps from ISO date strings, and pre-calculate `Date.now()` outside the loop rather than evaluating it for every item.
