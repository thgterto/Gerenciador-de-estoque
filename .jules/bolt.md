...
## 2025-03-04 - React Window ItemData Re-renders
**Learning:** When using `react-window` for virtualized lists, if the component defined for rows is inline or doesn't use `itemData` effectively, it can cause severe performance degradation due to unnecessary re-mounts on every scroll.
**Action:** Always define the row component outside the main render scope and pass all dependencies (like lists, callbacks, and selected IDs) through the `itemData` prop provided by `react-window`.
