...
## 2026-08-20 - Nested Array lookups inside Loops
**Learning:** Using `Array.find()` inside a loop (like `.forEach()`) creates an O(N * M) time complexity that can block the main thread for large datasets (like movement history mapping to inventory items).
**Action:** Always pre-compute a `Map` or dictionary for O(1) lookups before looping over datasets to reduce complexity to O(N + M).
