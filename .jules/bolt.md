...
## 2025-02-14 - Date Comparison Optimization
**Learning:** Calling `getItemStatus` with a `now` Date argument falls back to a legacy/explicit date comparison using `new Date()`. Omitting the `now` argument triggers a heavily optimized path that uses lexicographical string comparison against a cached ISO date string (`getTodayISO()`), which is ~10x faster.
**Action:** When filtering or calculating stats over large arrays using `getItemStatus`, do not pass the `now` argument unless explicitly required. This avoids O(N) Date allocations and utilizes the fast string comparison path.
