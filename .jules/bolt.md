...
## 2025-04-01 - [Avoid Virtualization Without Exact Specs]
**Learning:** Adding complex virtualization libraries (like react-window) to a component with highly variable row heights, especially across different screen sizes, without explicit layout specifications leads to broken UI and unexpected pagination removal. A simpler, safer approach is `React.memo` on the list component itself when its parent re-renders frequently due to local filter/state changes.
**Action:** When asked for a "small" performance optimization, prefer React built-ins (`memo`, `useMemo`, `useCallback`) over introducing or forcefully applying complex structural components like virtualized lists unless there is an explicit, clear bottleneck and exact design specs for row heights.
