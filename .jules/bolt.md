...
## 2025-02-14 - Intl.DateTimeFormat Caching
**Learning:** Calling `new Date(string).toLocaleDateString()` or `toLocaleString()` inside a loop to format dates is extremely slow because it instantiates a new locale formatter every time. In our performance benchmarking, caching an `Intl.DateTimeFormat` instance and calling its `.format(date)` method yielded a ~70x speedup.
**Action:** Always prefer caching a shared `Intl.DateTimeFormat` instance over repeatedly using `.toLocaleDateString()` or `.toLocaleString()` for high-volume formatting operations (such as rendering large lists/tables).
