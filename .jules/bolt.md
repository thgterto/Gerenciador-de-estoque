...
## 2025-02-14 - Date object creation in loop
**Learning:** Frequent creation of `Date` objects inside loops like `Array.map` or `Array.filter` adds measurable overhead to the main thread. Caching `Date.now()` outside the loops mitigates this overhead completely and maintains functionality when checking simple date expirations.
**Action:** When calculating remaining days or similar intervals using loop iterations, instantiate `const now = Date.now()` once before the loop and use `now` instead. Never write IIFEs in JSX solely for performance optimizations as it hurts readability.
