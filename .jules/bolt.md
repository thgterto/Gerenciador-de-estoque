...
## 2024-05-23 - React.memo defeat via complex props
**Learning:** In React components with virtualized/paginated lists, `React.memo` on row components is often defeated by passing new object references (e.g., inline styles `style={{width: '100%'}}`) or inline functions (`onClick={() => handler(id)}`). In this codebase, `InventoryList` was passing a `Set` for selected IDs and inline arrow functions, which caused the entire list of `InventoryGroupRow`/`InventoryChildRow` to re-render on *any* row selection change despite them being wrapped in `React.memo`.
**Action:** When inspecting list performance, verify that primitive values (booleans) are passed instead of complex data structures (like Sets) when possible, and ensure inline objects/functions are extracted or memoized to make `React.memo` effective.
