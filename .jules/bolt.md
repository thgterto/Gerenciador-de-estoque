...
## 2023-11-14 - ISO Date String Manipulation for Dashboard Analytics
**Learning:** Instantiating `new Date(string)` inside hot loops for analytics filtering and aggregation is extremely slow (approx 5x slower) compared to extracting UTC year/month components using `.slice()` on ISO 8601 strings. Using `string.startsWith()` for day-level grouping and `<` / `>` comparisons is much faster. Be mindful of potential local vs UTC timezone drift when replacing local date methods (`getFullYear()`) with ISO string slices, and construct boundary dates in UTC when slicing is used.
**Action:** When filtering or aggregating large arrays of historical items by date, prefer string comparisons and `.slice()` on ISO strings instead of creating `Date` objects inside the loop. Ensure you align search keys and array elements uniformly on UTC.
