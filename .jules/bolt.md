...
**Learning:** In lists that implement pagination or "load more" functionality (like `HistoryTable`), appending new items to the visible list state causes all previously rendered row components to re-render.
**Action:** Always wrap row components (e.g., `HistoryRow`, `HistoryMobileRow`) with `React.memo` when they rely on stable props (like a single data object) to prevent O(N) re-renders during list expansion.
