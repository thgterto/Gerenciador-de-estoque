...
## 2024-05-18 - [Hardcoded JWT Secret]
**Vulnerability:** Found a hardcoded JWT secret (`'supersecret_change_me_in_prod'`) in `server/src/config.ts`.
**Learning:** Hardcoded secrets, even if meant as fallbacks, create a critical risk of authentication bypass if the code is compromised or if an attacker guesses the default.
**Prevention:** Always require secrets to be injected via environment variables. If a fallback is absolutely necessary for out-of-the-box local usage (like in a portable app), generate a random secret at runtime using a secure random number generator (e.g., `crypto.randomBytes`) instead of a static string.
