...
**Vulnerability:** The `server/src/config.ts` file used a hardcoded fallback value (`supersecret_change_me_in_prod`) for the JWT secret if the `JWT_SECRET` environment variable was not set.
**Learning:** Defaulting to a hardcoded secret in production allows an attacker to forge JWT tokens and bypass authentication entirely if the environment variable is missing or misconfigured. This violates the principle of "Fail securely".
**Prevention:** Configuration files should fail to load (throw an error on startup) if critical secrets are missing rather than providing insecure defaults.
