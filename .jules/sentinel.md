...
## 2025-05-31 - [XSS in Print Modal]
**Vulnerability:** DOM-based XSS via `document.write` in QR code print functionality in `Modals.tsx`.
**Learning:** User-controlled data (`item.name`, `item.lotNumber`) was directly interpolated into an HTML string passed to `document.write()`.
**Prevention:** Always HTML-escape user inputs before inserting them into HTML templates, especially when using `document.write` or `innerHTML`.
