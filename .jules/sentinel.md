...
## YYYY-MM-DD - Removed Hardcoded JWT Secret
**Vulnerability:** A hardcoded JWT secret (`supersecret_change_me_in_prod`) was used as a fallback if `JWT_SECRET` was not provided in the environment. This would allow an attacker to forge JWT tokens if the application was run in production without the env var set.
**Learning:** Default fallback values for security credentials (especially JWT secrets or API keys) should never be static strings in the code.
**Prevention:** If an environment variable is not provided, dynamically generate a secure random secret using `crypto.randomBytes(32).toString("hex")` or throw an error on startup to enforce its configuration.
