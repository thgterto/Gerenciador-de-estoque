...
## 2026-05-12 - XSS in Print Modal
**Vulnerability:** User input was directly interpolated into HTML and rendered via `document.write` in the QR Code print modal.
**Learning:** Even internal tool interfaces (like print layouts) that don't execute scripts directly from the DB can still be a vector if an attacker creates an item with a malicious payload in its name or ID.
**Prevention:** Always escape HTML special characters when interpolating variables into raw HTML strings, especially when using legacy APIs like `document.write`.
