...
## 2025-02-28 - [Information Leakage in JWT Verification]
**Vulnerability:** Raw error objects from `request.jwtVerify()` were being sent directly to the client via `reply.send(err)`.
**Learning:** Sending raw error objects from authentication libraries can leak sensitive internal details or stack traces to potential attackers.
**Prevention:** Always catch authentication errors and return generic, sanitized error messages (e.g., `401 Unauthorized`) instead of raw error objects.
