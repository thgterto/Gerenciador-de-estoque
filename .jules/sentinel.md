...
## 2024-05-24 - Privilege Escalation in Auth Controller\n**Vulnerability:** The public /api/auth/register endpoint allowed unauthenticated users to supply a "role" field (e.g. "ADMIN") in the payload, granting them full administrative privileges via mass assignment.\n**Learning:** Always explicitly restrict and hardcode user roles in public-facing registration endpoints, rather than passing through unvalidated input.\n**Prevention:** Never allow the 'role' or permission-level fields to be set from user-provided data on a public endpoint; use safe defaults like 'USER'.
