...
**Vulnerability:** The backend server (`server/src/config.ts`) was using a hardcoded string ('supersecret_change_me_in_prod') as a fallback for `JWT_SECRET`. This would allow attackers to forge authentication tokens if the environment variable was missing.
**Learning:** Default fallback values for secrets should never be deterministic or easily guessable strings. Even in development environments, hardcoded secrets can be accidentally shipped to production.
**Prevention:** Always use cryptographically secure random values (e.g., `crypto.randomBytes(32).toString('hex')`) as fallbacks for required secrets, or explicitly crash the application on startup if the secret is missing.
