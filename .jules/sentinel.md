...
## 2023-10-24 - [Missing Security Headers Configuration]
**Vulnerability:** Fastify server lacked security header configuration (e.g. CSP, X-Frame-Options), falling prey to common OWASP A02 risks (Security Misconfiguration).
**Learning:** For a Fastify API backend without a traditional framework configuration (like `next.config.js`), the security scanner scripts may not be able to identify programmatic security headers in the code.
**Prevention:** Implement security headers using a global `onRequest` hook natively in Fastify instead of relying on external plugins or configuration files when a lightweight solution is preferable. This mitigates XSS, framing, and MIME-sniffing risks.
