...
## 2024-05-26 - [High] Cross-Site Scripting (XSS) in document.write for printing
**Vulnerability:** In `src/components/Modals.tsx`, untrusted variables (`item.name`, `item.lotNumber`, `item.id`) were injected directly into an HTML string passed to `printWindow.document.write(...)`. This created a DOM-based XSS vulnerability.
**Learning:** Developers often forget to sanitize user input when manually constructing DOM strings for new windows, especially when adding quick "print" functionality.
**Prevention:** Always use an HTML escaping function (like `escapeHtml`) when manually building HTML strings with variables, especially when using raw sinks like `document.write` or `innerHTML`.
