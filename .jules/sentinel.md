...
**Learning:** Hardcoded fallback secrets are dangerous because if an environment variable is missed in production, the application is vulnerable to token forgery using the known string.
**Prevention:** Use cryptographically secure random generation (e.g., `crypto.randomBytes(32).toString('hex')`) for runtime fallbacks, or fail securely on startup if the secret is missing.
