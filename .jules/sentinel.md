...
## 2025-02-18 - [Prevent XSS in document.write with inline html escaping]
**Vulnerability:** XSS vulnerability where user-controlled variables (like item name and ID) were directly injected into a template literal passed to `document.write` to open a print window for QR codes.
**Learning:** Even internal tool features like printing labels can be vulnerable to XSS if inputs aren't sanitized. `document.write` is highly susceptible to this.
**Prevention:** Instead of adding heavy sanitization libraries like `dompurify` just for a small print window (which can cause dependency bloat and is sometimes overkill for simple string interpolation), use a lightweight, focused HTML escape utility function that converts special characters (`&`, `<`, `>`, `"`, `'`) to their HTML entity equivalents before interpolation.
