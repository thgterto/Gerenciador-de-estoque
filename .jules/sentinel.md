...
## 2026-06-15 - [Replace hardcoded JWT fallback with dynamically generated secure secret]\n**Vulnerability:** A hardcoded fallback value was used for the JWT_SECRET parameter when the environment variable wasn't present. This compromises token validation if deployed to production without the setting configured, as anyone who accesses the repo could forge a valid JWT token.\n**Learning:** Default hardcoded cryptographic keys pose critical security risks for sensitive operations like token signing.\n**Prevention:** Use a runtime-generated cryptographically secure random value via the 'crypto' module for fallback keys. Print an explicit warning noting that server restarts will invalidate sessions to encourage correct environment variable configuration.
