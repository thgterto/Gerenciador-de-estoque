...
## 2025-05-28 - Missing Rate Limiting on Backend API
**Vulnerability:** The Fastify server (`server/src/app.ts`) did not have any rate limiting applied.
**Learning:** This leaves the API endpoints vulnerable to brute force and DoS attacks. The authentication endpoints like login or register could be abused without restrictions.
**Prevention:** Always implement a global or endpoint-specific rate limiter. In fastify, the `@fastify/rate-limit` plugin provides this defense layer.
