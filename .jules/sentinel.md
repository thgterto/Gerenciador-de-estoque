...
## 2026-03-14 - Fix XSS in QR Print Modal
**Vulnerability:** XSS risk due to `document.write` interpolation in `src/components/Modals.tsx` and `labcontrol-spfx/src/webparts/labControlApp/components/Modals.tsx`. If `item.name` or `item.lotNumber` contain malicious HTML (like `<script>`), it executes in the new window.
**Learning:** Even printing labels in a newly opened window via `document.write` is susceptible to DOM-based XSS if user inputs are not properly sanitized. The scanner flagged this pattern as dangerous.
**Prevention:** Always escape user-controlled variables (e.g., using a utility `escapeHtml` function to replace unsafe characters like `<`, `>`, `&`, `"`, and `'`) before injecting them into a dynamically generated HTML template or use DOM manipulation (`createElement`) instead of `document.write`.
