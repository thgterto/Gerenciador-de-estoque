...
## 2026-07-12 - [XSS] DOM-based XSS via document.write in Print Modals\n**Vulnerability:** DOM-based XSS was possible because user-supplied strings (`item.name`, `item.lotNumber`, `item.id`) were interpolated directly into an HTML string passed to `document.write()` when creating a print window.\n**Learning:** The use of `document.write()` with unsanitized data is a common vector for XSS, even in modern React applications when raw DOM manipulation is needed for specific features like printing.\n**Prevention:** Always sanitize data before injecting it into raw HTML strings, using a safe HTML escaper, or better yet, construct DOM elements securely using safe DOM APIs (`createElement`, `textContent`).
