...
## 2023-11-20 - Missing Input Validation on Inventory Endpoints
**Vulnerability:** The Fastify server (`server/src/adapters/controllers/InventoryController.ts`) previously casted `req.body` to `any` and blindly passed it to use-cases without validation.
**Learning:** Bypassing schema validation allows attackers to send malformed data, potentially triggering unexpected application behavior, database constraint violations, or injection attacks deeper in the stack.
**Prevention:** Always validate and sanitize external input at the controller layer before processing. Using schema validation libraries like `zod` ensures type safety and strips unexpected fields, providing defense in depth.
