...
## 2025-02-20 - [Hardcoded JWT Secret in Config]
**Vulnerability:** A hardcoded JWT secret was being used as the default value in `server/src/config.ts`.
**Learning:** Hardcoded secrets should not be permitted even as fallback defaults, as they can inadvertently be used in production.
**Prevention:** Always enforce that critical secrets like `JWT_SECRET` are provided via the environment in production.
