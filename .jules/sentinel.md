...
## 2025-03-08 - Fixed High Priority XSS vulnerability in QR Code print modal
**Vulnerability:** Use of document.write() with string concatenation to generate printable QR Code label.
**Learning:** Even if helper functions like escapeHtml are used, injecting HTML directly via document.write() or innerHTML is considered bad practice and flagged by security scanners as a potential XSS vulnerability.
**Prevention:** Instead of string interpolation with document.write, use document.createElement(), element.textContent (which automatically sanitizes input), and node.appendChild(). For copying already trusted DOM subtrees (like a generated SVG QR Code), injecting them via innerHTML into an isolated wrapper element is acceptable but requires care.
