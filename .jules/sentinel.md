...
## 2026-04-19 - DOM-based XSS in Print Label Generation
**Vulnerability:** `document.write` was used to generate HTML for a print window, directly interpolating raw item properties (name, lotNumber, id) without escaping.
**Learning:** Even internal tool interfaces must sanitize inputs when manually constructing HTML strings. The duplicated code in both main and SPFX components multiplied the risk.
**Prevention:** Use an `escapeHtml` utility for all manual string interpolations that inject variables into HTML context.
