...
**Vulnerability:** The Fastify server (`server/src/config.ts`) was using a hardcoded, weak default JWT secret (`'supersecret_change_me_in_prod'`) as a fallback if the environment variable was missing. Because the application is distributed as a portable local tool/executable, any user using the fallback could have their tokens forged, leading to privilege escalation or data tampering.
**Learning:** Hardcoded secrets in code are always a critical risk, but the risk is amplified in applications distributed to end-users (like Electron or packaged Node apps). Fallbacks must never be static values.
**Prevention:** If a secret is required but not provided via environment variables, the application should either fail to start securely OR automatically generate a cryptographically secure random value on the first run and persist it locally with restrictive file permissions (`0o600`).
