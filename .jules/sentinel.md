...
## 2025-05-28 - Information Leakage in Controller Error Handlers
**Vulnerability:** Fastify route controllers in `server/src/adapters/controllers/InventoryController.ts` intercepted errors via `try/catch` and forwarded the raw `error.message` directly to the client response (e.g., `res.status(500).send({ error: error.message })`).
**Learning:** Passing raw error messages to the client exposes internal system workings (e.g., database constraints, paths) which aids attackers in reconnaissance. This bypassed the global `errorHandler` designed to obscure such messages in production.
**Prevention:** Replace raw error message returns with safe, generic messages for the client (e.g., `'An internal server error occurred'`). Ensure the actual error object is logged internally (e.g., using `req.log.error(error)`) so developers can debug without exposing details to users.
