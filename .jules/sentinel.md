...
## 2024-10-27 - [CRITICAL] Code Injection in sqlite.exec()
**Vulnerability:** Code injection through unsanitized strings executing via `sqlite.exec()` instead of using `sqlite.prepare().run()`.
**Learning:** `exec()` allows multiple statements to be executed, which allows SQL injection. Better to use `prepare(stmt).run()`.
**Prevention:** Always use prepared statements, even when iterating over multiple statements.
