...
## 2026-06-25 - [XSS Fix in Modals Print Function]\n**Vulnerability:** XSS vulnerability in Modals.tsx components where user input was directly interpolated into a template string passed to `document.write`.\n**Learning:** The print functionality creates a new window and uses `document.write` with user-controlled data (item name, lot number). This is a classic DOM XSS vector if not escaped properly. Modals exist in both `src/components/Modals.tsx` and `labcontrol-spfx/src/webparts/labControlApp/components/Modals.tsx`.\n**Prevention:** Always sanitize/escape user data before injecting it into HTML contexts like `document.write` or `innerHTML`. Created a simple `escapeHTML` utility to safely handle variables.
