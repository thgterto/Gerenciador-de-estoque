...
## 2026-06-20 - Insecure Default JWT Secret
**Vulnerability:** The Fastify server (`server/src/config.ts`) used a hardcoded default JWT secret (`'supersecret_change_me_in_prod'`) if the `JWT_SECRET` environment variable was not set. This creates a critical risk where an attacker could forge tokens if deployed insecurely.
**Learning:** Relying on 'change me' placeholder secrets in fallback configuration paths exposes applications in production if the environment variables aren't strictly validated before startup.
**Prevention:** Fall back to cryptographically secure generated keys on startup using `crypto.randomBytes()` for token signing. Even if it forces reauthentication on restart, it prevents catastrophic token forgery.
