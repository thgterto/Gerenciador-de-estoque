...
## 2026-06-28 - XSS via document.write in Print Modals\n**Vulnerability:** DOM-based Cross-Site Scripting (XSS) vulnerability found in `QRGeneratorModal` (both in `src/components/Modals.tsx` and `labcontrol-spfx/src/webparts/labControlApp/components/Modals.tsx`). The print functionality used `printWindow.document.write` to dynamically generate HTML, interpolating properties like `item.name` and `item.lotNumber` without any sanitization.\n**Learning:** Direct interpolation of unsanitized user-controlled data into `document.write` or `innerHTML` is a critical XSS vector, even when rendering secondary outputs like print windows.\n**Prevention:** Always sanitize or HTML-escape variables before inserting them into an HTML string context. Use utility functions like `escapeHtml` (replacing `<, >, &, ", `\`` with HTML entities) to ensure data is treated strictly as text, not executable code.
