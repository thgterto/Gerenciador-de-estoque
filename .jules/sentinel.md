...
## 2025-05-26 - Predictable Token Forging via Hardcoded JWT Secret\n**Vulnerability:** The Fastify server used a hardcoded string `supersecret_change_me_in_prod` as a fallback for `JWT_SECRET` when not provided in the environment.\n**Learning:** Hardcoded secrets in codebases enable attackers to predictably forge authentication tokens if environment variables are not correctly configured.\n**Prevention:** Fallback to a securely generated random secret (`crypto.randomBytes(32).toString("hex")`) on startup so unconfigured instances are secured by default, even if it means sessions reset on restart.
