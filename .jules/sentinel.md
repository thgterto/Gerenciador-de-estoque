...
## 2023-10-24 - Information Leakage in Controllers
**Vulnerability:** Fastify route handlers were explicitly wrapping their execution in `try { ... } catch (error: any) { res.status(500).send({ error: error.message }); }`. This leaks potentially sensitive internal error messages to the client and bypasses Fastify's centralized `errorHandler` (which properly sanitizes 500 errors in production).
**Learning:** In Fastify (and many other async web frameworks), throwing an error or letting a rejected Promise propagate will automatically route the error through the global error handler. Over-catching at the controller level disables these central security/sanitization mechanisms.
**Prevention:** Avoid redundant `try/catch` blocks in controllers unless the controller needs to translate specific domain errors into safe API responses. Otherwise, let exceptions propagate to the framework's global error handler to ensure consistent and secure error formatting.
