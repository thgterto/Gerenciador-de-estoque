...
## 2026-06-22 - XSS via document.write in Print Window
**Vulnerability:** A Cross-Site Scripting (XSS) vulnerability was identified in `Modals.tsx` where user-controlled inputs (`item.name`, `item.lotNumber`, `item.id`) were interpolated directly into a template literal used in a `printWindow.document.write(...)` call without prior HTML sanitization/escaping.
**Learning:** `document.write` directly executes HTML strings, making it a critical sink for DOM-based XSS if the interpolated strings originate from or can be influenced by users.
**Prevention:** Always escape HTML entities (`&`, `<`, `>`, `"`, `'`) for any dynamic user data before placing it in an HTML context, even within auxiliary windows like print dialogs.
