...
## 2026-09-04 - XSS via document.write in Print Modals
**Vulnerability:** The application used `document.write()` in `Modals.tsx` to construct a new window for printing. While some inputs were escaped, `document.write` is an outdated API that is inherently risky and often flagged by security scanners as an XSS vector.
**Learning:** Security scanners and modern browser policies often distrust `document.write`. It circumvents DOM sanitization and can lead to code execution if unescaped data is passed.
**Prevention:** Avoid `document.write()` completely. Instead, use safer DOM manipulation techniques like setting `document.head.innerHTML` and `document.body.innerHTML` after opening the document, or use DOM methods like `createElement`.
