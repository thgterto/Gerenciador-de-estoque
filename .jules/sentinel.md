...
## 2026-08-30 - [Rate Limiting Added to Authentication]
**Vulnerability:** Missing rate limiting on sensitive endpoints (specifically /api/auth/login), leaving the application vulnerable to brute-force attacks.
**Learning:** Fastify's @fastify/rate-limit plugin can be used to easily add rate limiting. Setting `global: false` during registration allows for fine-grained control, applying limits only to specific routes like login.
**Prevention:** Always implement rate limiting on authentication and other sensitive endpoints to mitigate brute-force and denial-of-service attacks.
