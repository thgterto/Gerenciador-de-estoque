...
## 2026-03-30 - [Replace Hardcoded JWT Secret Fallback]
**Vulnerability:** The JWT Secret had a hardcoded default string ('supersecret_change_me_in_prod') if the JWT_SECRET environment variable was not set.
**Learning:** Defaulting to a hardcoded string makes local servers exposed, especially if they are inadvertently bound to 0.0.0.0 (all interfaces). It compromises the security of all instances.
**Prevention:** Always use a securely generated random string fallback (e.g., using crypto.randomBytes) when an environment variable for a secret is not provided. This ensures instances are protected by default.
