...
## 2026-07-13 - [Fix XSS in Label Printing Modal]
**Vulnerability:** XSS in QR Code print functionality. User-controlled variables (`item.name`, `item.lotNumber`, `item.id`) were being injected without sanitization into a `printWindow.document.write()` block in both React components and SharePoint framework components (`Modals.tsx`).
**Learning:** Even internal toolings or simple "print" features can introduce critical DOM XSS if they construct raw HTML strings and write them directly into new browser contexts.
**Prevention:** Always sanitize any dynamic or user-controlled input before concatenating it into an HTML template string, particularly when using `dangerouslySetInnerHTML`, `document.write`, or `innerHTML`.
