...
## 2026-04-06 - [Fix XSS in QR Code Printer]\n**Vulnerability:** Found XSS risk in Modals.tsx where document.write() directly interpolates user input (item.name, item.lotNumber, item.id) into an HTML template for a print window.\n**Learning:** The application prints QR Code labels using a raw HTML template string in document.write. Even though it's opening a new print window, this allows attackers to construct an item with a malicious name or lot number that could execute arbitrary JavaScript within the context of the application when the user clicks 'Print'.\n**Prevention:** Always escape user input before interpolating it into raw HTML templates, especially when using document.write() or dangerouslySetInnerHTML. An escapeHtml utility should be used or preferably use DOM building methods instead of raw HTML strings.
