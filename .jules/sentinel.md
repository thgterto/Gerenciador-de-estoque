...
## 2026-06-02 - Cross-Site Scripting (XSS) in HTML String Injection
**Vulnerability:** The `handlePrint` function used `printWindow.document.write()` to inject an HTML string dynamically constructed with user inputs (`item.name`, `item.lotNumber`, `item.id`) without any form of escaping or sanitization.
**Learning:** While React handles escaping for components correctly, building raw HTML strings using template literals and injecting them into the DOM directly bypasses React's security mechanisms entirely.
**Prevention:** Always implement HTML escaping when building and injecting raw HTML strings containing user input. Use an `escapeHtml` utility to sanitize common characters before insertion.
