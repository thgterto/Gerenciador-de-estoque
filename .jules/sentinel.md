...
## 2025-05-25 - JWT Forgery Risk via Hardcoded Secret
**Vulnerability:** The Fastify server (`server/src/config.ts`) was using a hardcoded fallback string (`'supersecret_change_me_in_prod'`) for the `JWT_SECRET`. If a production or staging environment failed to set the `JWT_SECRET` environment variable, attackers could forge JWT tokens using the known default secret to bypass authentication.
**Learning:** Default fallback secrets in security configs are a critical vulnerability, essentially creating a backdoor.
**Prevention:** Always use dynamically generated, cryptographically secure random values (e.g., `crypto.randomBytes(32).toString('hex')`) as fallbacks for critical secrets, failing securely and invalidating sessions on restart if the environment variable is missing.
