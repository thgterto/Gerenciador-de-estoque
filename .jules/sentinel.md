...
## 2023-11-20 - Prevent Information Exposure in Controllers
**Vulnerability:** The Fastify controllers manually caught errors and sent `res.status(500).send({ error: error.message });`. This directly leaked internal error messages (and potentially stack traces or query details) to the client, bypassing secure centralized error handlers.
**Learning:** Returning `error.message` directly to a client is an information disclosure risk, especially on 500 errors. Modern frameworks like Fastify can automatically catch rejected promises in `async` handlers and route them to a centralized error handler (like `ErrorHandler.ts`) which obfuscates error details in production.
**Prevention:** Remove manual `try/catch` blocks in async controller methods when the catch block only serves to return a 500 status with the error message. Let exceptions bubble up to the centralized secure error handler.
