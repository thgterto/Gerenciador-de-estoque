...
## 2025-06-03 - XSS in window.open document.write\n**Vulnerability:** XSS through unescaped variable interpolation in `document.write` inside `window.open`\n**Learning:** When generating print templates or new windows using `document.write`, any dynamic data (like `item.name`) must be HTML-escaped. React normally protects against this, but raw DOM APIs bypass those protections.\n**Prevention:** Create an `escapeHtml` utility function for any raw HTML interpolation, or use DOM APIs like `document.createElement`.
