...
## 2024-03-29 - [Fix XSS in QR Code Generator]
**Vulnerability:** Untrusted user input (`item.name`, `item.lotNumber`, `item.expiryDate`, `item.id`) was being directly interpolated into an HTML string passed to `document.write()` when generating a printable QR Code label. This bypassed React's default XSS protections, creating an XSS risk.
**Learning:** Even in React applications, manual DOM manipulation methods like `document.write()` or `innerHTML` (or `dangerouslySetInnerHTML`) can re-introduce XSS vulnerabilities if user data is not explicitly sanitized. The fact that the same component existed in two parallel directories (`src/` and `labcontrol-spfx/src/webparts/`) meant both needed to be patched.
**Prevention:** Always sanitize or escape user input before manually injecting it into the DOM. Use a dedicated `escapeHtml` utility (or a library like DOMPurify) to encode special characters (`<`, `>`, `&`, `"`, `'`) when building HTML strings.
