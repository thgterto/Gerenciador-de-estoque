...
## 2024-05-13 - [Hardcoded JWT Secret Fallback Removed]
**Vulnerability:** A hardcoded default JWT secret string ('supersecret_change_me_in_prod') was present in `server/src/config.ts`. If administrators failed to set the `JWT_SECRET` environment variable in production, attackers could potentially forge JWT tokens by exploiting the public fallback.
**Learning:** Hardcoded fallbacks for cryptographic keys represent a significant security flaw. Even if intended only for development, developers often forget to configure production environments properly.
**Prevention:** Always use random ephemeral fallback values for cryptographic secrets if they aren't explicitly configured. Here, `crypto.randomBytes(32).toString('hex')` was used to ensure that even a server with a missing configuration file remains secure against token forgery, simply trading token persistence for security until the environment variable is properly set.
