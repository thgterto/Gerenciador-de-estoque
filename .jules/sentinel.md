...
## 2025-06-26 - Overly Permissive CORS Configuration
**Vulnerability:** The Fastify server was configured with `origin: '*'` in `server/src/app.ts`, allowing any external website to make cross-origin requests to the local API. If a user visits a malicious website while the local tool is running, the malicious site could make requests to the local API (e.g., `http://127.0.0.1:3000`) and interact with the user's local data or trigger actions on their behalf. This is a severe risk for a local tool.
**Learning:** While `origin: '*'` is often used during initial development for convenience or when a tool is purely local, it breaks the isolation expected of a local application if the user's browser is also accessing the internet. A malicious site can exploit this overly permissive CORS policy.
**Prevention:** For local tools, the CORS policy should strictly whitelist `http://localhost:<port>` and `http://127.0.0.1:<port>`, or simply limit origins based on an environment configuration, denying all others by default. Never use `origin: '*'` for a service intended for local/desktop use if it handles sensitive data or state modifications.
