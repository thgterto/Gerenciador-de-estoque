...
## 2025-02-15 - [🛡️ Sentinel: Fix DOM-based XSS in QR Code Generator Print Window]
**Vulnerability:** A DOM-based XSS vulnerability was found in the `QRGeneratorModal` component within both `src/components/Modals.tsx` and `labcontrol-spfx/src/webparts/labControlApp/components/Modals.tsx`. Unsanitized user inputs (`item.name`, `item.lotNumber`, `item.id`) were being directly interpolated into an HTML string and rendered using `printWindow.document.write()`.
**Learning:** `document.write` can be a significant XSS risk if user-controlled variables are included without escaping, especially in dynamically generated print windows.
**Prevention:** Implement a basic HTML escaping function (replacing `&`, `<`, `>`, `"`, `'`) and apply it to all user-controlled text strings injected into DOM insertion points.
