...
## 2023-10-27 - Information Exposure in Error Handling
**Vulnerability:** Fastify route handlers were returning the raw `error.message` on 500 errors to the client, which could leak internal implementation details, paths, or database structures.
**Learning:** Even though `adapters/ErrorHandler.ts` had a production check, caught errors returned directly within `InventoryController.ts` bypassed the global handler. Always ensure controllers log errors server-side and only expose generic messages like "Internal Server Error" externally.
**Prevention:** Avoid `res.status(500).send({ error: error.message });` in controllers. Either throw the error to leverage a global secure error handler, or explicitly catch, log `req.log.error(error)`, and send a sanitized response.
