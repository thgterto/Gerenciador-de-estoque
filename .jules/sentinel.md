...
## 2025-05-28 - Restrict Fastify CORS Configuration
**Vulnerability:** Fastify server was configured to allow all origins via `app.register(cors, { origin: '*' })`, which exposes the API to Cross-Origin Resource Sharing attacks, especially when running locally.
**Learning:** Default wildcard CORS configs are dangerous even for local development servers, as visiting a malicious website on the same machine could allow it to make unauthorized requests to local services.
**Prevention:** Always restrict CORS to the exact origins that need access, such as `localhost`, `127.0.0.1`, and the `file://` protocol for Electron apps. Ensure safe URL parsing using a `try-catch` block to handle malformed origins.
