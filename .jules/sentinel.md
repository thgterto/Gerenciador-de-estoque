...
## 2026-03-20 - XSS Vulnerability in Print Modal
**Vulnerability:** The `QRGeneratorModal` component in `src/components/Modals.tsx` and `labcontrol-spfx/src/webparts/labControlApp/components/Modals.tsx` interpolated user input (`item.name`, `item.lotNumber`, `item.id`) directly into an HTML template string passed to `printWindow.document.write()`. This introduced a Cross-Site Scripting (XSS) vulnerability.
**Learning:** Using `document.write()` with unescaped user input is extremely dangerous. Even if the input originates from a database or API, it must be sanitized or escaped before being rendered as HTML. The `react-qr-code` package generates SVGs safely, but the surrounding HTML constructed manually for printing was vulnerable.
**Prevention:** Always escape HTML entities (`<`, `>`, `&`, `"`, `'`) when interpolating user-controlled strings into raw HTML contexts like `document.write()` or `dangerouslySetInnerHTML`. A simple utility function `escapeHtml` was added to mitigate this.
