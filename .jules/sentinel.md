...
## 2025-03-27 - Remove Hardcoded JWT Secret
**Vulnerability:** A hardcoded fallback string ('supersecret_change_me_in_prod') was used for the JWT secret in `server/src/config.ts`. If deployed without setting the `JWT_SECRET` environment variable, an attacker could trivially forge valid JWTs and gain unauthorized access to the application.
**Learning:** Hardcoded default secrets are a critical vulnerability. Even as a fallback, developers often deploy to production without changing them.
**Prevention:** Use a securely generated random string (e.g. `crypto.randomBytes(32).toString('hex')`) as a fallback to ensure that if the secret isn't provided, it is at least unguessable (though it will reset on server restart, providing an incentive to set the environment variable properly while maintaining security).
