...
## 2026-08-28 - Explicit Error Catching Bypassing Global Error Handler
**Vulnerability:** The `InventoryController.ts` methods explicitly wrapped their execution in `try/catch` blocks and returned `res.status(500).send({ error: error.message });`. This bypassed the central Fastify error handler (`ErrorHandler.ts`) which is designed to sanitize error messages (e.g., return "Internal Server Error" in production instead of raw database messages or stack traces).
**Learning:** Even if a global error handler is registered, developers may unintentionally bypass it by handling errors locally within controllers and emitting unstructured/unsafe error messages. This can leak sensitive internal state such as SQL syntax or backend architectures.
**Prevention:** In modern Node.js web frameworks like Fastify and Express 5, async route handlers automatically pass unhandled promise rejections to the global error middleware. Avoid redundant try-catch blocks in route handlers unless you are handling a very specific error gracefully; let the global handler manage unexpected errors consistently.
