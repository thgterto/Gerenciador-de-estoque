...
## 2024-05-24 - [Remove Hardcoded JWT Secret]
**Vulnerability:** A fallback JWT secret ('supersecret_change_me_in_prod') was hardcoded in `server/src/config.ts`.
**Learning:** Hardcoded fallback secrets, even when seemingly intended for development, pose a critical risk if deployed to production without the environment variable being set. It allows trivial token forgery and complete authentication bypass.
**Prevention:** When providing fallbacks for cryptographic secrets, always use dynamically generated random values (e.g., `crypto.randomBytes`) accompanied by a clear warning log, ensuring that missing configurations result in session invalidation across restarts rather than a compromised system.
