...
## 2024-05-24 - Hardcoded JWT Secret Removed
**Vulnerability:** The Fastify API used a hardcoded fallback JWT secret (`supersecret_change_me_in_prod`) in `server/src/config.ts`.
**Learning:** Hardcoded secrets in source code or packaged binaries (like the ones built by `pkg`) allow attackers to forge tokens if they gain access to the file.
**Prevention:** Generate a random, cryptographically secure secret on first run and persist it locally alongside the application's data.
