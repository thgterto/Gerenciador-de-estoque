...
## 2026-03-16 - [XSS via document.write in QR Generator]\n**Vulnerability:** Found XSS vulnerability in QR Code print function where item properties (name, lotNumber, id) were directly injected into `document.write()` HTML string without sanitization.\n**Learning:** The application uses dynamic HTML generation for printing labels in a new window. Even internal data displayed in this manner can execute malicious scripts if not escaped.\n**Prevention:** Always sanitize/escape dynamic variables before inserting them into HTML strings, particularly when using `document.write()` or `innerHTML`.
