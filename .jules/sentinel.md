...
## 2025-02-28 - [Hardcoded JWT Secret]
**Vulnerability:** A hardcoded fallback string `'supersecret_change_me_in_prod'` was used for the JWT secret in `server/src/config.ts`.
**Learning:** Hardcoding cryptographic secrets creates a critical vulnerability if the fallback is used in production.
**Prevention:** Remove hardcoded fallbacks and enforce that the environment variable must be provided for production secrets. Throw an error if it's missing or generate a secure random default strictly for development if appropriate, but never a hardcoded fallback string.
