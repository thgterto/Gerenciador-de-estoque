...
## 2025-05-26 - XSS Risk in Document.Write
**Vulnerability:** The `handlePrint` function in `src/components/Modals.tsx` and `labcontrol-spfx/src/webparts/labControlApp/components/Modals.tsx` used `printWindow.document.write` with unescaped user data (`${item.name}`).
**Learning:** Using `document.write` is inherently dangerous if user input is interpolated directly without HTML escaping. This leads to DOM-based XSS.
**Prevention:** Always escape HTML entities (`&`, `<`, `>`, `"`, `'`) before writing user data into the DOM, or prefer using standard DOM manipulation APIs (`createElement`, `textContent`) instead of constructing raw HTML strings.
